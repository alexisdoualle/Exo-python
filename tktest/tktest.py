...
class Main():
    def __init__(self, master):
        ...
        self.text["text"]="               Ajouter un contact"

        #campos de texto
        self.lab=Label(frame, text="Nom")
        self.lab.pack(side=LEFT)
        self.lab.grid(row=1)

        self.name=Entry(frame, text="Nom", width=30)
        self.name.insert(0,"Entrez un nom")
        self.name.pack()
        self.name.grid(row=1)
        self.name.bind("<FocusIn>",self.clearName)

        self.phone=Entry(frame, text="Téléphone",width=30)
        self.phone.insert(0,"Entrez un numéro de téléphone")
        self.phone.pack()
        self.phone.grid(row=2)
        self.phone.bind("<FocusIn>",self.clearPhone)

        #botoes
        self.btn=Button(frame, text="Ajouter", command=self.adicionar)
        self.btn.pack()
        self.btn.grid(row=5,rowspan=1,sticky="w")

        self.attbtn=Button(frame, text="Editer", command=self.atualizar)
        self.attbtn.pack()
        self.attbtn.grid(row=5,rowspan=1,sticky="s")

        self.delbtn=Button(frame,text="Supprimer",command=self.apagar)
        self.delbtn.pack()
        ...
        self.conec=sqlite3.connect('contacts.db')
...
#inicia janela
app = Tk()
app.title("Agenda")#titulo da janela
Main(app)
app.mainloop()
